...
class FewShotREFramework:
    ...
    def train(self,
        ...
        iter_right_dis = 0.0
        iter_sample = 0.0
        loop = tqdm(range(start_iter, start_iter + train_iter), leave=False)
        for it in loop:
            if pair:
            ...
            if self.adv:
                ...
                loop.set_description(f"Iter: [{it}/{start_iter + train_iter}]")
                loop.set_postfix(Loss = iter_loss / iter_sample, Accuracy=100 * iter_right / iter_sample, dis_loss = iter_loss_dis / iter_sample, dis_acc = 100 * iter_right_dis / iter_sample)
            else:
              loop.set_description(f"Iter: [{it}/{start_iter + train_iter}]")
              loop.set_postfix(Loss = iter_loss / iter_sample, Accuracy=100 * iter_right / iter_sample)
    ...
    def eval(self,
        ...
        iter_sample = 0.0
        with torch.no_grad():
            loop = tqdm(range(1, eval_iter+1), leave=False)
            for it in loop:
                if pair:
                ...
                loop.set_description(f"Iter: [{it}/{eval_iter}]")
                loop.set_postfix(Accuracy=100 * iter_right / iter_sample)
